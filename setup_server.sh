...
#   - Instala y configura Django, Gunicorn, PostgreSQL, Redis, Nginx, Celery, y (opcional) Certbot SSL.
...
#   - Instala requirements en un entorno virtual.
...
# nano setup_server.sh && sudo chmod +x /home/pablollh/setup_server.sh && sudo /home/pablollh/setup_server.sh
#
# Notas:
#   - Configurado para usuario 'pablollh' y grupo 'ai_huntred'.
#   - Usa e2-medium (4 GB RAM) con disco de 20 GB.
#   - Incluye swap de 4 GB para mitigar problemas de memoria.
#   - Certbot es opcional y requiere que el dominio apunte a la VM (34.57.227.244).
############################################################################################
...
su - "$APP_USER" -c "source $VENV_DIR/bin/activate && \
    ...
    pip install wheel && \
    pip install django gunicorn psycopg2-binary redis celery django-celery-beat django-celery-results && \
    cd $PROJECT_DIR && [ -f requirements.txt ] && pip install -r requirements.txt || true"
...
# Certbot (opcional)
if [ "$DOMAIN" != "localhost" ] && nslookup "$DOMAIN" >/dev/null 2>&1; then
    certbot --nginx -d "$DOMAIN" -m "$EMAIL" --agree-tos --non-interactive --redirect || echo "Certbot falló, verifica DNS."
else
    echo "Omitiendo Certbot: dominio no resuelve o es localhost."
fi
...
su - "$APP_USER" -c "source $VENV_DIR/bin/activate && cd $PROJECT_DIR && \
    ...
    python manage.py loaddata $PROJECT_DIR/app/fixtures/initial_data.json && \
...
echo "Silk: http://$DOMAIN/silk/ (usa el mismo usuario admin)"
echo
echo "Si Certbot funcionó, usa HTTPS: https://$DOMAIN"
echo "==========================================================="