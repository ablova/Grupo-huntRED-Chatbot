...
PROPOSAL_STATUS_CHOICES = [('DRAFT', 'Borrador'), ('SENT', 'Enviada'), ('ACCEPTED', 'Aceptada'), ('REJECTED', 'Rechazada')]

# Estados para Contratos
CONTRATO_STATUS_CHOICES = [('PENDING_APPROVAL', 'Pendiente de Aprobación'), ('APPROVED', 'Aprobado'),('SIGNED', 'Firmado'), ('REJECTED', 'Rechazado')]

# Eventos que activan hitos de pago
TRIGGER_EVENT_CHOICES = [('CONTRACT_SIGNING', 'Firma del contrato'), ('START_DATE', 'Fecha de inicio'),('MILESTONE_1', 'Hitos de proyecto'), ('CUSTOM_EVENT', 'Evento personalizado')]

# Estados de pagos
PAYMENT_STATUS_CHOICES = [('PENDING', 'Pendiente'), ('PAID', 'Pagado'),('OVERDUE', 'Vencido'), ('CANCELLED', 'Cancelado')]

# Estados de Feedback
FEEDBACK_STATUS_CHOICES = [('PENDING', 'Pendiente'), ('COMPLETED', 'Completado'), ('SKIPPED', 'Omitido')]

# Tipos de Feedback
FEEDBACK_TYPE_CHOICES = [('INTERVIEW', 'Entrevista'), ('CANDIDATE', 'Candidato'),('PROPOSAL', 'Propuesta'), ('HIRE', 'Contratación')]

# Resultados de Feedback
FEEDBACK_RESULT_CHOICES = [('YES', 'Sí'), ('NO', 'No'),('PARTIAL', 'Parcial'), ('NOT_APPLICABLE', 'No Aplica')]
...
NOTIFICATION_CHANNEL_CHOICES = [('WHATSAPP', 'WhatsApp'), ('TELEGRAM', 'Telegram'), ('EMAIL', 'Email'),('SLACK', 'Slack'), ('NTFY', 'ntfy.sh')]

# Estados de Notificación
NOTIFICATION_STATUS_CHOICES = [('PENDING', 'Pendiente'), ('SENT', 'Enviada'), ('DELIVERED', 'Entregada'),('READ', 'Leída'), ('FAILED', 'Fallida')]

# Tipos de Notificación
NOTIFICATION_TYPE_CHOICES = [('FEEDBACK', 'Feedback'), ('MATCHING', 'Matching'), ('PROPOSAL', 'Propuesta'),('HIRE', 'Contratación'), ('SYSTEM', 'Sistema')]
...
class NotificationChannel(models.Model):
    ...
    CHANNEL_CHOICES = [('email', 'Email'), ('whatsapp', 'WhatsApp'), ('push', 'Notificación push'),('sms', 'SMS'), ('slack', 'Slack'), ('teams', 'Microsoft Teams'),('webhook', 'Webhook'),]
    business_unit = models.ForeignKey('BusinessUnit', on_delete=models.CASCADE, related_name='notification_channels', help_text="Business Unit asociada a este canal")
    channel = models.CharField(max_length=20, choices=CHANNEL_CHOICES, help_text="Canal de notificación")
    enabled = models.BooleanField(default=True, help_text="¿Este canal está habilitado para esta BU?")
    config = models.JSONField(default=dict, help_text="Configuración específica del canal (tokens, URLs, etc.)")
    priority = models.IntegerField(default=10, help_text="Prioridad del canal (menor número = mayor prioridad)")
...
class Notification(models.Model):
    ...
    recipient = models.ForeignKey('Person', on_delete=models.CASCADE, related_name='notifications', help_text="Destinatario de la notificación")
    notification_type = models.CharField(max_length=20, choices=NOTIFICATION_TYPE_CHOICES, help_text="Tipo de notificación")
    channel = models.CharField(max_length=20, choices=NOTIFICATION_CHANNEL_CHOICES, help_text="Canal utilizado para enviar la notificación")
    status = models.CharField(max_length=20, choices=NOTIFICATION_STATUS_CHOICES, default='PENDING', help_text="Estado de la notificación")
    content = models.TextField(help_text="Contenido de la notificación")
    metadata = models.JSONField(default=dict, help_text="Datos adicionales de la notificación")
...
class Feedback(models.Model):
    ...
    # Relaciones
    candidate = models.ForeignKey('Person', on_delete=models.CASCADE, related_name='feedbacks', help_text="Candidato asociado al feedback")
    
    interviewer = models.ForeignKey('Person', on_delete=models.SET_NULL, null=True, related_name='given_feedbacks', help_text="Entrevistador que proporcionó el feedback")
...
class RecoveryAttempt(models.Model):
    """
    Modelo para rastrear los intentos de recuperación de usuarios inactivos.
    
    Este modelo registra cuándo un usuario ha sido identificado como inactivo
    y los intentos realizados para recuperar su atención.
    """
    ...
    user = models.ForeignKey(
        ...
        help_text="Usuario que está siendo monitoreado"
    )
    ...
    status = models.CharField(
        ...
        help_text="Estado actual del intento de recuperación"
    )
    
    attempts = models.PositiveIntegerField(
        default=0,
        help_text="Número de intentos de recuperación realizados"
    )
    ...
    last_attempt = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Último intento de recuperación"
    )
    
    first_attempt = models.DateTimeField(
        auto_now_add=True,
        help_text="Primer intento de recuperación"
    )
    
    ended_at = models.DateTimeField(
        null=True,
        blank=True,
        help_text="Cuándo finalizó el proceso de recuperación"
    )
    
    metadata = models.JSONField(
        default=dict,
        blank=True,
        help_text="Datos adicionales sobre los intentos de recuperación"
    )
    ...
    def __str__(self):
        return f"{self.user} - {self.get_status_display()} ({self.attempts}/{self.max_attempts} intentos)"
    
    def mark_as_recovered(self):
        """Marca el intento como exitosamente recuperado"""
        self.status = self.RecoveryStatus.RECOVERED
        self.ended_at = timezone.now()
        self.save(update_fields=['status', 'ended_at'])
    
    def mark_as_abandoned(self):
        """Marca el intento como abandonado"""
        self.status = self.RecoveryStatus.ABANDONED
        self.ended_at = timezone.now()
        self.save(update_fields=['status', 'ended_at'])
    ...
    def record_attempt(self):
        ...
        # Si superamos el máximo de intentos, marcamos como abandonado
        if self.attempts >= self.max_attempts:
            self.mark_as_abandoned()
        else:
...
class ConsultorInteraction(models.Model):
    """
    Registra las interacciones de los consultores con los candidatos.
    Permite hacer seguimiento de acciones como mensajes directos, reinicios de conversación, etc.
    """
    class ActionType(models.TextChoices):
        DIRECT_MESSAGE = 'direct_message', 'Mensaje Directo'
        RESET_CONVERSATION = 'reset_conversation', 'Reinicio de Conversación'
        STATUS_UPDATE = 'status_update', 'Actualización de Estado'
        NOTE_ADDED = 'note_added', 'Nota Agregada'
        FOLLOW_UP = 'follow_up', 'Seguimiento Programado'
    
    consultor = models.ForeignKey(
        'Person',
        on_delete=models.CASCADE,
        related_name='consultor_actions',
        help_text="Consultor que realizó la acción"
    )
    
    candidate = models.ForeignKey(
        'Person',
        on_delete=models.CASCADE,
        related_name='consultor_interactions',
        help_text="Candidato con quien se interactuó"
    )
    
    action_type = models.CharField(
        max_length=50,
        choices=ActionType.choices,
        help_text="Tipo de acción realizada por el consultor"
    )
    
    details = models.JSONField(
        default=dict,
        help_text="Detalles adicionales de la interacción"
    )
    ...
    class Meta:
        verbose_name = 'Interacción de Consultor'
        verbose_name_plural = 'Interacciones de Consultores'
        ordering = ['-created_at']
        indexes = [
            models.Index(fields=['consultor', 'action_type']),
            models.Index(fields=['candidate', 'created_at']),
            models.Index(fields=['action_type', 'created_at']),
        ]
    
    def __str__(self):
        return f"{self.get_action_type_display()} - {self.consultor} -> {self.candidate}"
    
    @classmethod
    def log_interaction(cls, consultor, candidate, action_type, details=None):
        """
        Registra una nueva interacción de un consultor con un candidato.
        
        Args:
            consultor: Instancia de Person (debe ser un consultor)
            candidate: Instancia de Person (candidato)
            action_type: Uno de los valores de ActionType
            details: Diccionario con detalles adicionales de la interacción
            
        Returns:
            La instancia de ConsultorInteraction creada
        """
    ...
    @classmethod
    def get_candidate_interactions(cls, candidate, days=30):
        """
        Obtiene todas las interacciones de un candidato en los últimos N días.
        
        Args:
            candidate: Instancia de Person (candidato)
            days: Número de días hacia atrás para buscar interacciones
            
        Returns:
            QuerySet de interacciones ordenadas por fecha descendente
        """
        since = timezone.now() - timezone.timedelta(days=days)
        return cls.objects.filter(
            candidate=candidate,
            created_at__gte=since
        ).select_related('consultor').order_by('-created_at')
    
    @classmethod
    def get_consultor_activity(cls, consultor, days=7):
        """
        Obtiene el resumen de actividad de un consultor en los últimos N días.
        
        Args:
            consultor: Instancia de Person (consultor)
            days: Número de días hacia atrás para buscar interacciones
            
        Returns:
            Diccionario con el conteo de interacciones por tipo
        """
        since = timezone.now() - timezone.timedelta(days=days)
...
class EstadoPago(models.TextChoices):
    ...
    EN_PROCESO = 'en_proceso', 'En Proceso de Pago'
    COMPLETADO = 'completado', 'Completado'
    RECHAZADO = 'rechazado', 'Rechazado'
