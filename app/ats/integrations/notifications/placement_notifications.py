...
_placement_notifier = None

def get_placement_notifier():
    """Obtiene una instancia singleton del servicio de notificaciones de colocaciones.
    
    Utiliza lazy loading para evitar consultar la base de datos durante la importación
    del módulo.
    """
    global _placement_notifier
    if _placement_notifier is None:
        from app.models import BusinessUnit
        business_unit = BusinessUnit.objects.first()
        if business_unit:
            _placement_notifier = PlacementNotificationService(business_unit)
        else:
            # Fallback si no hay business_unit disponible
            _placement_notifier = PlacementNotificationService(None)
    return _placement_notifier

# Alias para mantener compatibilidad con código existente
placement_notifier = get_placement_notifier