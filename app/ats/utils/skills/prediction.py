...
def create_skill_predictor(model: BaseEstimator, feature_names: List[str]) -> Dict[str, Any]:
    """
    Crea un predictor de habilidades.
    ...
    Returns:
        Diccionario con el predictor configurado
    """
    return {
        'model': model,
        'feature_names': feature_names,
        'predict': lambda features: _predict_skills(model, features, feature_names)
    }

def _predict_skills(model: BaseEstimator, features: np.ndarray, 
                   feature_names: List[str]) -> List[Skill]:
    """
    Predice habilidades a partir de características.
    ...
    Returns:
        Lista de habilidades predichas
    """
    predictions = model.predict_proba(features)
    skills = []
    
    for i, pred in enumerate(predictions):
        if pred[1] > 0.5:  # Umbral de confianza
            skill = Skill(
