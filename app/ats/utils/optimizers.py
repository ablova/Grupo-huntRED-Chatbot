...
def initialize_optimizers() -> Dict[str, bool]:
    ...
    try:
        ...
        results['gpt_connector'] = register_optimizer(
            'gpt_connector',
            'app.ats.chatbot.optimized_connectors',
        ...
        try:
            from app.ats.import_config import register_handler
            
            if results['gpt_connector']:
                register_handler('gpt_handler', 'app.ats.chatbot.optimized_connectors', 'OptimizedGPTConnector')
                logger.info("GPT optimizado registrado en el sistema de importación")
                
            if results['nlp_connector']:
                register_handler('nlp_processor', 'app.ats.chatbot.optimized_connectors', 'OptimizedNLPConnector')
