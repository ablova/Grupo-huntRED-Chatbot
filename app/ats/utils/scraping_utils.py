...
class PlaywrightAntiDeteccion:
    ...
    # Cache para perfiles de navegador
    _profile_cache = {}
    ...
    async def generar_perfil_navegador(plataforma="default"):
        ...
        cache_key = f"profile_{plataforma}"
        cached_profile = PlaywrightAntiDeteccion._profile_cache.get(cache_key)
        
        if cached_profile and time.time() - cached_profile.get('timestamp', 0) < PlaywrightAntiDeteccion._cache_ttl:
        ...
        # Usar UserAgentRotator para obtener un User-Agent consistente
        from app.ats.utils.user_agent_rotator import UserAgentRotator
        user_agent_rotator = UserAgentRotator()
        user_agent = await user_agent_rotator.get_user_agent(
            device_type='desktop',
            browser='chrome' if platform == "Windows" else 'safari'
        )
        ...
        # Guardar en caché
        PlaywrightAntiDeteccion._profile_cache[cache_key] = {
            'data': profile,
            'timestamp': time.time()
        }
    ...
    async def simular_comportamiento_humano(page: Page):
        ...
        try:
            # Usar asyncio.gather para ejecutar acciones en paralelo cuando sea posible
            ...
            # Optimizar la selección de elementos
            elementos = await page.query_selector_all('a[href], button:not([disabled])')
            if elementos:
    ...
    async def obtener_cookies_formato_playwright(cookies_dict, dominio):
        """Convierte cookies a formato Playwright de manera optimizada."""
        cookies_playwright = []
