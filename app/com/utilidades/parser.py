...
class CVParser:
    ...
    def _filter_huntred_skills(self, skills: Dict) -> Dict:
        ...
        return {
            "technical": [s for s in skills["technical"] if s.lower() in HUNTRED_TECH_SKILLS],
            "soft": [s for s in skills["soft"] if s.lower() in HUNTRED_SOFT_SKILLS],
            "tools": [s for s in skills["tools"] if s.lower() in HUNTRED_TOOLS],
            "certifications": [s for s in skills["certifications"] if s.lower() in HUNTRED_CERTS]
        }
    
    def _filter_huntu_skills(self, skills: Dict) -> Dict:
        """Filtra skills específicas para Huntu."""
        return {
            "technical": [s for s in skills["technical"] if s.lower() in HUNTU_TECH_SKILLS],
            "soft": [s for s in skills["soft"] if s.lower() in HUNTU_SOFT_SKILLS],
            "tools": [s for s in skills["tools"] if s.lower() in HUNTU_TOOLS],
            "certifications": [s for s in skills["certifications"] if s.lower() in HUNTU_CERTS]
        }
    
    def _filter_sexsi_skills(self, skills: Dict) -> Dict:
        """Filtra skills específicas para SEXSI."""
        return {
            "technical": [s for s in skills["technical"] if s.lower() in SEXSI_TECH_SKILLS],
            "soft": [s for s in skills["soft"] if s.lower() in SEXSI_SOFT_SKILLS],
            "tools": [s for s in skills["tools"] if s.lower() in SEXSI_TOOLS],
            "certifications": [s for s in skills["certifications"] if s.lower() in SEXSI_CERTS]
        }
